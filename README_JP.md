...
| コマンド | 説明 | 使用方法 |
|---------|------|----------|
| `/set_canal` | ボットがニュースを送信するチャンネルを設定 | `/set_canal [チャンネル:#ニュース]` |
| `/dashboard` | フィルター設定パネルを開く | `/dashboard` |
| `/setlang` | サーバーのボット言語を設定 | `/setlang 言語:ja_JP` |
| `/forcecheck` | 即時スキャンを強制実行 | `/forcecheck` |
| `/clean_state` | state.jsonの一部をクリーンアップ (自動バックアップ付き) | `/clean_state タイプ:dedup 確認:はい` |

### 📊 情報コマンド

| コマンド | 説明 | 使用方法 |
|---------|------|----------|
| `/status` | ボットの統計を表示 | `/status` |
| `/feeds` | 監視中の全ソースをリスト表示 | `/feeds` |
| `/help` | ヘルプマニュアルを表示 | `/help` |
| `/ping` | ボットのレイテンシを確認 | `/ping` |
| `/about` | ボット情報 | `/about` |

> **🔒 権限:** 管理者のみが管理者コマンドを使用できます。
